...
def recognition(face_image):
    ...
    # Open camera 
def main():
    global isThread, score, name, caption
    

    cap = cv2.VideoCapture('dongseofinal.mp4')
    start = time.time_ns()
    frame_count = 0
    fps = -1
    ...
    writer = cv2.VideoWriter('./static/results/dongseofinal1.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 30.0, (width, height))
    a= 0
    ...
    while(True):
        ...
        for i in range(len(bboxs)):
            ...
            if isThread == True:
                ...
                # a = Process(target=recognition, args=(face_image,))
                # score ,name = recognition(face_image)
                # Recognition
                # face_image.append(frame[y1:y2, x1:x2])
         
                a =Thread(target=recognition, args=(face_image,))
            ...
            if score < 0.05:
                caption= "UN_KNOWN"
                mosaic_img = cv2.resize(frame[y1:y2, x1:x2], dsize=(0, 0), fx=0.04, fy=0.04) # 축소
                mosaic_img = cv2.resize(mosaic_img, (x2-x1, y2-y1), interpolation=cv2.INTER_AREA)  # 확대
                frame[y1:y2, x1:x2] = mosaic_img
            else:
